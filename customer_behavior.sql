...
select subscription_status, count(customer_id) as total_customer, sum(purchase_amount) 
 as total_revenue, round(avg(purchase_amount),2) as avg_spend
from customer 
group by subscription_status;

#Q6. Which 5 products have the highest percentage of purchases with discounts applied?
select item_purchased, 
round(sum(case when discount_applied ='Yes' then 1 else 0 end)*100/count(0),2) as discount_rate
from customer
group by item_purchased
...
select item_rnk, category, item_purchased ,total_order
from item_count
where item_rnk<=3;
